 ...
 */
	...
	SPI_clock_enable_disabled(1, 1);

	/* Initialization of SPI2 */
	SPI_Initialization(1, SPI_FULL_DUPLEX_MODE, SPI_MODE_MASTER, SPI_CPOL_IDLE_LOW, SPI_CPHA_FIRST_CLK_TRANSITION_CAPTURE,
                       SPI_DATA_FRAME_FORMAT_8, SPI_SCK_BAUD_RATE_PRESCALER_2, SPI_SSM_ENABLE,
                       0, 0);

	char data_send[] = "Hello World";
	char data_recv[20];
	SPI_Send_Data(1, data_send, 12, data_recv);
