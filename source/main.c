...
void ejercicio2()
{
	...
	while(1) {
		...
		enable_disable_SPI_peripheral(SPI2_PERIPHERAL, false);
	}

}
#endif

void ejercicio2_alternative()
{
	/* Initiation of GPIO module to load the button configuration */
	GPIO_initialization_module();
	...
	/* GPIO pins from SPI2 peripheral */
	/* PB-10 como SCLK */
	GPIO_activate_alternate_functionality(GPIO_PORT_B, GPIO_PIN_10, 5);
	...
	/* GPIO pins from SPI3 peripheral */
	/* PB-3 como SCLK */
	GPIO_activate_alternate_functionality(GPIO_PORT_B, GPIO_PIN_3, 6);

	/* PB-4 como MISO */
	GPIO_activate_alternate_functionality(GPIO_PORT_B, GPIO_PIN_4, 6);

	/* PB-5 como MOSI */
	GPIO_activate_alternate_functionality(GPIO_PORT_B, GPIO_PIN_5, 6);

	/* PA-15 como NSS */
	GPIO_activate_alternate_functionality(GPIO_PORT_A, GPIO_PIN_15, 5);
	...
	/* Initialization of SPI3 */
	/*SPI_Initialization(SPI3_PERIPHERAL, SPI_FULL_DUPLEX_MODE, SPI_MODE_SLAVE,
					   SPI_CPOL_IDLE_LOW, SPI_CPHA_FIRST_CLK_TRANSITION_CAPTURE,
					   SPI_DATA_FRAME_FORMAT_8, SPI_SCK_BAUD_RATE_PRESCALER_32,
					   SPI_SSM_DISABLE, 0, 0);
	enable_disable_SPI_peripheral(SPI3_PERIPHERAL, true);*/
	...
	while(1) {
		if(GPIO_read_pin(GPIO_PORT_A, GPIO_PIN_0)) {
			char data_send[] = "Hello World";
			uint8_t data_len = strlen(data_send);
			SPI_load_data_send(SPI2_PERIPHERAL, (uint8_t *)data_send, data_len);
		}
