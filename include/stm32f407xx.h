...
/* Para cada elemento que cuelga de los perifericos, anotamos sus direcciones
 * base. */
/* AHB1 - GPIOs */
...
/* Para acceder a todos los registros de un un periférico, en vez de definir
todas las direcciones para los diferentes periféricos, podemos generar una
estructura que contenga varios elementos/variables (que hagan referencia a los
registros del periférico). Luego simplemente cogeremos esa estructura como un
puntero, y le pondremos la dirección base del periférico. De esta manera
podremos acceder facilmente a los periféricos sin necesidad de estar generando
todas las direcciones una a una.

Es muy importante que las variables que empleemos TENGAN EL MISMO TAMAÑO QUE LOS
REGISTROS, YA QUE SINO GENERAREMOS UN DESCUADRE EN LOS ACCESOS Y LAS
ASIGNACIONES.
*/

/* Consejo, añadir una breve descripción de la finalidad de cada variable. De
 * este modo se sabrá para que vale cada uno */
/* Aunque NO SON NECESARIOS EN TODOS LOS CAMPOS, al hacer referencia a valores
 * que pueden modificarse en cualquier momento y sin necesidad de hacer nada
 * nosotros, es muy recomendable incluir el modificador volatile en todos los
 * campos; o al menos en los de entrada de valores. */
...
typedef struct {
  ...
  volatile uint32_t
      AHB1LPENR; // RCC AHB1 peripheral clock enable in low power mode register
  volatile uint32_t
      AHB2LPENR; // RCC AHB2 peripheral clock enable in low power mode register
  volatile uint32_t
      AHB3LPENR; // RCC AHB3 peripheral clock enable in low power mode register
  volatile uint32_t reserved_7;
  volatile uint32_t
      APB1LPENR; // RCC APB1 peripheral clock enable in low power mode register
  volatile uint32_t
      APB2LPENR; // RCC APB2 peripheral clock enable in low power mode register
...
#define RCC_APB1_EN_RESET_I2C3 ((uint32_t)0x00800000)
#define RCC_APB1_EN_RESET_I2C2 ((uint32_t)0x00400000)
#define RCC_APB1_EN_RESET_I2C1 ((uint32_t)0x00200000)
